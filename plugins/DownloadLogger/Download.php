...
class Download extends BaseEntity
{
    ...
    public function save()
    {
        ...
        $this->validate();
        $sql = "insert into download (item_id, guest_name, guest_ip, added) values (?, ?, ?, now())";
    ...
    {
        $res = $this->getDb()->exec("select * from download where id=?", array($id));
    ...
    {
        $comment = new self();
        $res = $comment->getDb()->exec("select * from download where item_id=?", array($item->id));
        $result = array();
